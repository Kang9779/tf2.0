...
from CycleGan import Generator, Discriminator, cycle_consistency_loss, generator_loss, discriminator_loss
...
def train(train_datasetA, train_datasetB, epochs, lsgan=True, cyc_lambda=10):
    ...
    for epoch in range(epochs):
        ...
        with tf.GradientTape() as genA2B_tape, tf.GradientTape() as genB2A_tape, \
            ...
            discA_loss = discriminator_loss(
                discA_real_output, discA_fake_output, lsgan=lsgan)
            discB_loss = discriminator_loss(
                discB_real_output, discB_fake_output, lsgan=lsgan)
        ...
        genA2B_gradients = genA2B_tape.gradient(
            genA2B_loss, genA2B.trainable_variables)
        genB2A_gradients = genB2A_tape.gradient(
            genB2A_loss, genB2A.trainable_variables)

        discA_gradients = discA_tape.gradient(
            discA_loss, discA.trainable_variables)
        discB_gradients = discB_tape.gradient(
            discB_loss, discB.trainable_variables)
        ...
        if epoch % 40 == 0:
            generate_images(trainA, trainB, genB2A_output,
                            genA2B_output, epoch)
...
if __name__ == '__main__':
    train(train_datasetA, train_datasetB, epochs=epochs,
          lsgan=True, cyc_lambda=cyc_lambda)
